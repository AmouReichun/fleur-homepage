...
const BRANDS = [
  ...
  },
  {
    ...
    type: "美容室",
    copy: "大人女性に支持される技術を、\nあなたの強みに。",
    strengths: ["白髪ぼかし", "髪質改善", "艶カラー", "大人女性向け"],
    seo: "高知市 美容師求人",
  ...
  },
  {
    ...
    type: "アイラッシュサロン",
    copy: "好きな美容を、好きな仲間と、\n好きな働き方で。",
    strengths: ["まつ毛パーマ", "LEDエクステ", "眉WAX", "全席半個室"],
    seo: "高知市 アイリスト求人",
...
/* ───────── スタッフ紹介（写真・名前・役職は実データ。声はサンプル＝後で差し替え） ───────── */
const STAFF_VOICES = [
  {
    name: "川上 凛", role: "スタイリスト", brand: "fleurami", years: "美容師歴4年",
    image: "/images/admin/staff-2-1782195045005.jpg",
    reason: "「艶カラーの仕上がりに憧れて」見学に来たのが入社のきっかけ。",
    joy: "お客様の『可愛い！』の一言が何よりのやりがいです。",
    holiday: "休日はカフェ巡りと、新しいカラーの研究をしています。",
    goal: "髪質改善で指名される、頼られるスタイリストになること。",
  },
  {
    name: "山岡 悠弥", role: "店長 / トップスタイリスト", brand: "fleurami", years: "美容師歴14年",
    image: "/images/admin/staff-1-1782195037653.jpg",
    reason: "技術と人を育てられる環境に惹かれて。",
    joy: "スタッフの成長を間近で見られること。",
    holiday: "家族と過ごす時間と、外部講習でのインプット。",
    goal: "香南市で一番『働きたい』と言われるサロンに。",
  },
  {
    name: "西田 ななみ", role: "スタイリスト", brand: "Riv.by fleurami", years: "美容師歴3年",
    image: "/images/admin/staff-6-1782195072738.jpg",
    reason: "大人女性向けの上質な接客を学びたくて入社しました。",
    joy: "白髪ぼかしでお客様が明るい表情になる瞬間。",
    holiday: "美術館やインテリアショップで感性をリフレッシュ。",
    goal: "デザインカラーでも指名いただけるようになりたい。",
  },
  {
    name: "細川 彩香", role: "スタイリスト", brand: "Riv.by fleurami", years: "美容師歴8年",
    image: "/images/admin/staff-4-1782195059188.jpg",
    reason: "長く続けられる働き方ができると感じたから。",
    joy: "『ここに来ると元気になる』と言ってもらえること。",
    holiday: "ゆっくり読書とおいしいごはん。",
    goal: "後輩が安心して相談できる先輩でいること。",
  },
  {
    name: "やすい", role: "アイリスト", brand: "Raffine", years: "",
    image: "/images/admin/staff-7-1782195080215.jpg",
    reason: "半個室で自分のペースで施術に集中できる環境が魅力でした。",
    joy: "目元が変わるだけで自信を持ってもらえること。",
    holiday: "韓国コスメとカフェ巡りでトレンド収集。",
    goal: "まつ毛パーマのデザインで指名される存在に。",
  },
  {
    name: "おざき", role: "アイリスト", brand: "Raffine", years: "",
    image: "/images/admin/staff-8-1782195085415.jpg",
    reason: "同世代の仲間と楽しく働けそうだと思って。",
    joy: "『また来ます』と笑顔で帰ってもらえる時間。",
    holiday: "友達とランチ、たまに眉毛の勉強会。",
    goal: "眉WAXまでトータルで提案できるアイリストへ。",
  },
];

/* ───────── 数字で見る（実数値は確認中。受領後に差し替え予定） ───────── */
const STATS: { label: string; value: string; suffix?: string }[] = [
  { label: "スタッフ平均年齢", value: "" },
  { label: "女性スタッフ比率", value: "", suffix: "%" },
  { label: "有給取得率", value: "", suffix: "%" },
  { label: "育休取得実績", value: "", suffix: "名" },
  { label: "店舗数", value: "3", suffix: "店舗" },
  { label: "月平均新規来店数", value: "", suffix: "名" },
  { label: "次回予約率", value: "", suffix: "%" },
  { label: "平均退勤時間", value: "" },
];
...
/* ───────── キャリアプラン ───────── */
const CAREER_HAIR = ["アシスタント", "スタイリスト", "副店長", "店長", "マネージャー"];
const CAREER_EYE = ["アシスタント", "アイリスト", "リーダー", "副店長", "店長"];

/* ───────── 募集要項 ───────── */
const REQUIREMENTS = [
  { label: "募集職種", value: "美容師（スタイリスト／アシスタント）・アイリスト" },
  { label: "給与", value: "経験・能力により優遇。詳細は見学・面談時にご案内します。" },
...
const HIRING_ORG = {
  "@type": "Organization",
  name: "fleur GROUP",
  sameAs: "https://fleurami-group.jp",
};
const datePosted = "2026-06-01";
const validThrough = "2027-06-30";
function jobPosting(title: string, desc: string, locality: string, occupation: string) {
  return {
...
const jobPostings = [
  jobPosting(
    "美容師（スタイリスト・アシスタント）",
    "香南市の美容室fleuramiで美容師を募集。髪質改善・カット・艶カラーが学べる地域密着型サロン。香南市 美容室求人。完全週休2日制・社会保険完備・アシスタントから育成。未経験・ブランクOK。",
    "香南市",
    "美容師"
  ),
  jobPosting(
    "美容師（大人女性向けサロン）",
    "高知市の美容室Riv.by fleuramiで美容師を募集。白髪ぼかし・髪質改善・艶カラーが強みの大人女性向けサロン。高知市 美容師求人。完全週休2日制・社会保険完備。",
    "高知市",
    "美容師"
  ),
  jobPosting(
    "アイリスト",
    "高知市のまつげサロンRaffineでアイリストを募集。まつ毛パーマ・LEDエクステ・眉WAX。全席半個室、20代女性スタッフ中心。高知市 アイリスト求人。未経験歓迎。",
    "高知市",
    "アイリスト"
  ),
];
      ...
      <main className="bg-white text-site-text overflow-hidden">
        ...
        <section className="relative">
          ...
          <div className="max-w-3xl mx-auto px-5 pt-16 sm:pt-24 pb-10 text-center">
            ...
            <h1 className="font-serif text-[26px] leading-[1.5] sm:text-4xl sm:leading-[1.5] font-medium text-balance animate-fadeinup-slow">
              「ここで働きたい」と
              <br className="sm:hidden" />
              思える場所で。
            </h1>
            <p className="mt-6 text-sm sm:text-base text-site-muted leading-loose animate-fadeinup-late">
            ...
            <div className="mt-9 flex flex-col sm:flex-row gap-3 justify-center animate-fadeinup-late">
              <a href="#entry" className="bg-site-accent text-white px-8 py-3.5 text-sm font-medium tracking-wider rounded-sm hover:bg-opacity-90 transition-all">
                サロン見学はこちら
              </a>
              <a href={stickyInstagram} target="_blank" rel="noopener noreferrer" className="border border-site-greige text-site-text px-8 py-3.5 text-sm font-medium tracking-wider rounded-sm hover:border-site-accent transition-all">
                Instagramで見る
              </a>
            </div>
          </div>
              ...
              <Reveal key={b.key} delay={i * 80}>
                <div className="relative h-[64vw] max-h-[440px] sm:h-[360px] rounded-2xl overflow-hidden group">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-6 sm:p-9">
                    ...
                    <p className="font-serif text-white text-xl sm:text-3xl font-medium leading-[1.55] whitespace-pre-line text-balance">
                      {b.copy}
                    </p>
        ...
        <section className="bg-site-light py-20 sm:py-28 px-5">
          <div className="max-w-4xl mx-auto">
            <Reveal className="text-center mb-12 sm:mb-16">
              ...
              <p className="text-sm sm:text-base text-site-muted leading-loose max-w-2xl mx-auto">
        ...
        <section className="py-20 sm:py-28 px-5">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="text-center text-[11px] text-site-muted/70 mt-8">※スタッフの声は一例です。実際の声は順次掲載予定です。</p>
          </div>
        </section>
              ...
              {[
                ...
                <Reveal key={col.title}>
                  ...
                  <ol className="relative border-l border-site-greige ml-2">
                    {col.data.map((d) => (
                      <li key={d.time} className="relative pl-6 pb-7 last:pb-0">
        ...
        <section className="py-20 sm:py-28 px-5">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {[
                { title: "美容師", steps: CAREER_HAIR },
                { title: "アイリスト", steps: CAREER_EYE },
              ].map((c) => (
                <Reveal key={c.title}>
                  ...
                  <div className="flex flex-col items-center gap-0">
                    {c.steps.map((step, i) => (
                      <div key={step} className="w-full flex flex-col items-center">
                        <div className="w-full max-w-xs text-center bg-white border border-site-greige rounded-full py-3 text-sm font-medium shadow-sm">
                          {step}
                        </div>
              ...
              {REQUIREMENTS.map((r, i) => (
                <Reveal key={r.label} delay={i * 40}>
                  <div className="flex flex-col sm:flex-row gap-1 sm:gap-4 px-6 py-5 border-b border-site-greige last:border-b-0">
                    ...
                    <p className="text-sm text-site-text leading-relaxed">{r.value}</p>
                  </div>
                </Reveal>
              ))}
            </div>
            <p className="text-center text-[11px] text-site-muted/70 mt-5">※給与・勤務時間などの詳細は、見学・面談時に丁寧にご案内します。</p>
          </div>
        </section>
            ...
            <div className="divide-y divide-site-greige border-y border-site-greige">
              {FAQ.map((f) => (
                <details key={f.q} className="group py-5">
                  ...
                  <p className="mt-4 text-sm text-site-muted leading-loose pl-5 border-l border-site-accent/40">{f.a}</p>
                </details>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-2xl mx-auto text-center">
            <Reveal>
              ...
              <p className="text-sm text-white/70 leading-loose mb-10">
                「ちょっと気になる」で大丈夫です。
                <br />
                応募の前に、まずは雰囲気を見にきてください。
                <br className="sm:hidden" />
                Instagramのお気軽なDMでもお待ちしています。
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center mb-14">
                {BRANDS.map((b) => (
                  <a key={b.key} href={b.instagram} target="_blank" rel="noopener noreferrer"
                    className="border border-white/30 text-white px-5 py-3 text-xs font-medium tracking-wide rounded-sm hover:border-white transition-all">
                    {b.name} のInstagram
                  </a>
                ))}
              </div>
